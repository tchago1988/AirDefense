...
from code.Cons import (WIN_WIDTH, WINDOW_HEIGHT, FPS, COLOR_WHITE, COLOR_YELLOW, COLOR_GREEN, COLOR_RED,
                       COLOR_BLUE, ENEMY_EVENT, MENU_OPTION, ENTITY_SCORE, )
...
class Level:
    def __init__(self, window, game_mode, stage_number=1):
        ...
        # Stage manager controls stage data and timer.
        ...
        # Base health according to stage and game mode.
    ...
    def play_shoot_sound(self):  # Plays the shooting sound.
    ...
    def play_explosion_sound(self):  # Plays the explosion sound.
    ...
    def run(self):  # Main stage loop.
        ...
        while True:
            ...
            # Draw only the pause screen while paused.
            if self.paused:
                self.draw_entities()
                self.draw_hud()
                self.draw_pause()
            ...
            if not self.game_over and not self.win:
                ...
                # Wait for ENTER after win or game over.
                self.draw_entities()
    ...
    def update_entities(self):  # Updates and draws all entities.
        for ent in self.entity_list[:]:
            ...
            if self.stage == 3 and isinstance(ent, Enemy):
                # Enemies shoot only in the upper half of the screen.
    ...
    def check_collisions(self):  # Checks all gameplay collisions.
        remove_list = []
        for ent in self.entity_list:
                    ...
                    if isinstance(enemy, Enemy):
                        if ent.rect.colliderect(enemy.rect):
                            ...
                            if enemy.health <= 0:
                                ...
                                self.entity_list.append(Explosion('Explosion', enemy.rect.center))
                ...
                for enemy_missile in self.entity_list:
                    if isinstance(enemy_missile, EnemyMissile):
                        if ent.rect.colliderect(enemy_missile.rect):
                            ...
                            self.play_explosion_sound()
                            self.entity_list.append(Explosion('Explosion', enemy_missile.rect.center))
            # Enemy missile logic.
            if isinstance(ent, EnemyMissile):
                ...
                for player in self.players:
                    if ent.rect.colliderect(player.rect):
                        remove_list.append(ent)
                        # Enemy missiles damage the base.
    ...
    def check_game_state(self):  # Checks if the player has won or lost the stage.
        # The player loses if the base health reaches zero.
        if self.base_health <= 0:
            self.game_over = True
            pygame.mixer.music.stop()
            pygame.time.set_timer(ENEMY_EVENT, 0)
        # Check the stage timer.
        if self.stage_manager.is_stage_finished():
            # The player wins only if the base is alive
            # and at least one enemy was destroyed.
            if self.base_health > 0 and self.enemies_destroyed > 0:
                self.win = True
            else:
                self.game_over = True
            pygame.mixer.music.stop()
    ...
    def get_level_result(self, status):  # Returns the final level result.
        return {
            'status': status,
            'stage': self.stage,
    ...
    def draw_hud(self):  # Draws level information on the screen.
        self.draw_text(f'FASE: {self.stage}', (10, 10), COLOR_YELLOW)
        if self.game_mode == MENU_OPTION[0]:
            self.draw_text(f'PONTOS: {self.score}', (10, 35), COLOR_WHITE)
        ...
        self.draw_text(f'TEMPO: {self.stage_manager.get_remaining_time()}', (WIN_WIDTH - 130, 10), COLOR_WHITE)
        self.draw_text('P - PAUSAR', (WIN_WIDTH - 130, 35), COLOR_YELLOW)
        self.draw_controls()
        if self.win:
            self.draw_text_center('MISSAO CUMPRIDA!', WINDOW_HEIGHT // 2, COLOR_GREEN)
            self.draw_text_center('ENTER - RESULTADO', WINDOW_HEIGHT // 2 + 35, COLOR_WHITE)
        if self.game_over:
            self.draw_text_center('BASE DESTRUIDA!', WINDOW_HEIGHT // 2, COLOR_RED)
            self.draw_text_center('ENTER - RESULTADO', WINDOW_HEIGHT // 2 + 35, COLOR_WHITE)

    def draw_controls(self):  # Draws control instructions.
        if self.game_mode == MENU_OPTION[0]:
            self.draw_text_center_small('CONTROLES:  ← → MOVER   |   M ATIRAR   |   P PAUSAR', WINDOW_HEIGHT - 28,
                                        COLOR_WHITE)
        else:
            self.draw_control_box_p2_left()
            self.draw_control_box_p1_right()

    def draw_control_box_p2_left(self):  # Draws Player 2 control instructions.
        box_width = 190
        box_height = 70
        box_x = 10
        ...
        self.draw_text_small('A + D MOVER', (box_x + 20, box_y + 30), COLOR_WHITE)
        self.draw_text_small('SPACE ATIRAR', (box_x + 20, box_y + 53), COLOR_YELLOW)

    def draw_control_box_p1_right(self):  # Draws Player 1 control instructions.
        box_width = 180
        box_height = 70
        ...
        self.draw_text_small('← → MOVER', (box_x + 20, box_y + 30), COLOR_WHITE)
        self.draw_text_small('M ATIRAR', (box_x + 20, box_y + 53), COLOR_YELLOW)
    ...
    def draw_pause(self):  # Draws the pause overlay.
        ...
        self.draw_text_center('PAUSE', WINDOW_HEIGHT // 2 - 40, COLOR_YELLOW)
        self.draw_text_center('PRESSIONE P PARA CONTINUAR', WINDOW_HEIGHT // 2 + 10, COLOR_WHITE)
        self.draw_text_center('ESC - VOLTAR AO MENU', WINDOW_HEIGHT // 2 + 55, COLOR_RED)

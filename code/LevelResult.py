...
import sys
import pygame
from code.Cons import (WIN_WIDTH, WINDOW_HEIGHT, FPS, ASSET_PATH, MENU_OPTION, COLOR_WHITE,
                       COLOR_YELLOW, COLOR_GREEN, COLOR_RED, COLOR_CYAN)
...
class LevelResult:
    def __init__(self, window, result, has_next_stage):
        ...
        self.background = pygame.image.load(f'{ASSET_PATH}{background_name}.png').convert()
    ...
    def draw_screen(self):  # Draws the complete result screen.
        ...
        if is_final_stage and self.result.get('new_record', False):   self.draw_text_center('NOVO RECORDE!', 24,
                                                                                        COLOR_YELLOW, 88)
        # Draw the top separator line.
        pygame.draw.line(self.window, COLOR_CYAN, (100, 125), (WIN_WIDTH - 100, 125), 2)
        y = 150
        # Draw score information for 1P mode.
        if game_mode == MENU_OPTION[0]:
            self.draw_stat('Pontuação Level',
                           self.result['score'], y)
            y += 38
        # Draw score information for 2P mode.
        else:
            self.draw_stat('Pontuação P1',
                           self.result['score_p1'], y)
            y += 38

            self.draw_stat('Pontuação P2',
                           self.result['score_p2'], y)
        ...
        self.draw_stat('Pontuação Total',
                       self.result['total_score'], y)
        y += 38
        # Draw the best score only on the final stage.
        if is_final_stage:
            self.draw_stat('Melhor Resultado',
                           self.result['best_score'], y)
            y += 38
        y += 6
        # Draw enemy statistics.
        self.draw_stat('Inimigos abatidos',
                       self.result['enemies_destroyed'], y)
        y += 38
        self.draw_stat(
            'Sucesso inimigo',
            self.result['enemy_success'], y)
        y += 38
        # Draw missile defense statistics only on Level 3.
        if stage == 3:
            self.draw_stat(
                'Mísseis abatidos',
                self.result['missiles_destroyed'], y)
            y += 38
        # Draw launched missile statistics.
        self.draw_stat(
            'Mísseis lançados',
            self.result['missiles_launched'], y)
